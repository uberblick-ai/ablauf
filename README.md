...
consumes. The repo's `auth` fixture, rendered
(`fixtures/text/auth.mmd` + `fixtures/spike/positions.json`):

<p align="center">
  <img src="docs/assets/readme-auth.svg" width="480" alt="The auth fixture rendered by ablauf: a top-down flowchart with stadium, decision and process nodes">
</p>
...
edits — first a leaf node, then a whole three-node MFA branch (the
`auth-add-leaf` and `auth-add-branch` directive sets from
`fixtures/acceptance/directives.json`, applied cumulatively):

| v1 — the base chart | v2 — `metrics` added | v3 — MFA branch added |
| :---: | :---: | :---: |
| ![The base auth chart](docs/assets/readme-auth.svg) | ![The same chart with an added metrics leaf; every existing node unmoved](docs/assets/readme-auth-add-leaf.svg) | ![The same chart with a three-node MFA branch; every existing node unmoved](docs/assets/readme-auth-add-branch.svg) |

The promise **across** versions of a chart is deliberately not byte-equality —
the canvas grows, new nodes and edges appear, v3 is 151px wider than v1. It is
*recognizability*: every node that existed before is drawn at exactly the
coordinates it had, so the chart your reader learned last week is still the
chart they see this week, just with more on it. Byte-equality is the promise
**within** a version: same text plus same store renders the same SVG,
byte for byte, on every replica. These images are generated by
[`scripts/readme-images.mjs`](scripts/readme-images.mjs), which refuses to
write them if any pre-existing node moved between frames.
