...
description: Issue adversary role for one prepared ablauf issue, run as the issue-preparer's nested subagent; acts only when given its role, its own run identity, the exact issue, and the parent preparer's run id.
---
